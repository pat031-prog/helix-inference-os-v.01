param(
    ...
    [int]$Tokens = 2200,
    [string]$OutputDir = "verification/nuclear-methodology/branch-pruning-forensics",
    [string]$RunId = ""
)
...
if ($BranchDepth -lt 2) {
    throw "BranchDepth must be at least 2."
}
...
if ([string]::IsNullOrWhiteSpace($RunId)) {
    $RunId = "branch-pruning-forensics-$stamp"
}
$logPath = Join-Path $verificationDir "local-branch-pruning-forensics-suite-$stamp.log"
$manifestPath = Join-Path $verificationDir "local-branch-pruning-forensics-suite-$stamp-run.json"
$stableLogPath = Join-Path $verificationDir "local-branch-pruning-forensics-suite.log"
$stableManifestPath = Join-Path $verificationDir "local-branch-pruning-forensics-suite-run.json"
...
try {
    ...
    Write-Host "[helix] Branch depth: $BranchDepth"
    ...
    $pyArgs = @(
        "tools\run_branch_pruning_forensics_suite_v1.py",
        "--output-dir", $verificationDir,
        "--run-id", $RunId,
        "--case", $Case,
        "--depth", [string]$Depth,
        "--branch-depth", [string]$BranchDepth,
    ...
    $candidate = Join-Path $verificationDir "local-branch-pruning-forensics-suite-$RunId.json"
    ...
    $manifest = [ordered]@{
        artifact = "local-branch-pruning-forensics-suite-run"
        generated_by = "tools/run_branch_pruning_forensics_suite_secure.ps1"
        run_id = $RunId
        ...
        claim_boundary = "Policy-layer branch pruning over native Merkle DAG topology, not automatic descendant tombstoning."
    }

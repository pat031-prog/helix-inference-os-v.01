param(
    ...
    [int]$Timeout = 240,
    [string]$RunId = "",
    [string]$OutputDir = "verification\nuclear-methodology\cloud-provider-substitution-longitudinal"
)
...
if ([string]::IsNullOrWhiteSpace($RunId)) {
    $RunId = "cloud-provider-longitudinal-$stamp"
}
...
$logPath = Join-Path $verificationDir "local-cloud-provider-substitution-longitudinal-$stamp.log"
$manifestPath = Join-Path $verificationDir "local-cloud-provider-substitution-longitudinal-$stamp-run.json"
$stableLogPath = Join-Path $verificationDir "local-cloud-provider-substitution-longitudinal.log"
$stableManifestPath = Join-Path $verificationDir "local-cloud-provider-substitution-longitudinal-run.json"
$artifactSlug = "local-cloud-provider-substitution-longitudinal-v1"
...
try {
    ...
    $manifest = [ordered]@{
        artifact = "local-cloud-provider-substitution-longitudinal-run"
        generated_by = "tools/run_cloud_provider_substitution_longitudinal_secure.ps1"
        run_id = $RunId
        ...
        claim_boundary = "Observed DeepInfra API evidence only. No semantic truth or global non-equivocation claim."
    }

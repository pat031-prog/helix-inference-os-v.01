param(
    ...
    [int]$Rounds = 14,
    [int]$EventRound = 5,
    [int]$TokensPerTurn = 2400,
    [int]$AuditTokens = 3800,
    [string]$OutputDir = "verification/nuclear-methodology/cognitive-drift-rollback",
    [string]$RunId = ""
)
...
if ([string]::IsNullOrWhiteSpace($RunId)) {
    $RunId = "cognitive-drift-rollback-$stamp"
}
$logPath = Join-Path $verificationDir "local-cognitive-drift-rollback-$stamp.log"
$manifestPath = Join-Path $verificationDir "local-cognitive-drift-rollback-$stamp-run.json"
$stableLogPath = Join-Path $verificationDir "local-cognitive-drift-rollback.log"
$stableManifestPath = Join-Path $verificationDir "local-cognitive-drift-rollback-run.json"
...
try {
    ...
    Write-Host "[helix] Event round: $EventRound"
    ...
    $pyArgs = @(
        "tools\run_cognitive_drift_rollback_v1.py",
        "--output-dir", $verificationDir,
        "--models", $Models,
        "--auditor-model", $AuditorModel,
        "--rounds", [string]$Rounds,
        "--event-round", [string]$EventRound,
    ...
    $candidate = Join-Path $verificationDir "local-cognitive-drift-rollback-$RunId.json"
    ...
    $transcriptMarkdownCandidate = Join-Path $verificationDir "local-cognitive-drift-rollback-$RunId-transcript.md"
    ...
    $transcriptJsonlCandidate = Join-Path $verificationDir "local-cognitive-drift-rollback-$RunId-transcript.jsonl"
    ...
    $manifest = [ordered]@{
        artifact = "local-cognitive-drift-rollback-run"
        generated_by = "tools/run_cognitive_drift_rollback_secure.ps1"
        run_id = $RunId
        ...
        claim_boundary = "Qualitative cloud-only tombstone rollback drift. No adversarial threat, sentience, local .hlx identity, or numerical KV<->SSM transfer claim."
    }

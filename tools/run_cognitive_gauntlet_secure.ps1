param(
    ...
    [ValidateSet("gauntlet", "kusanagi", "drift", "kusanagi-nuke", "drift-nuke")]
    [string]$Scenario = "gauntlet",
    [int]$Rounds = 8,
    [int]$AttackRound = 4,
    [int]$TokensPerTurn = 1400,
    [int]$AuditTokens = 2600,
    [string]$OutputDir = "verification/nuclear-methodology/cognitive-gauntlet",
    [string]$RunId = ""
)
...
if ([string]::IsNullOrWhiteSpace($RunId)) {
    $RunId = "cognitive-$Scenario-$stamp"
}
$logPath = Join-Path $verificationDir "local-cognitive-gauntlet-$stamp.log"
$manifestPath = Join-Path $verificationDir "local-cognitive-gauntlet-$stamp-run.json"
$stableLogPath = Join-Path $verificationDir "local-cognitive-gauntlet.log"
$stableManifestPath = Join-Path $verificationDir "local-cognitive-gauntlet-run.json"
...
try {
    ...
    Write-Host "[helix] Attack round: $AttackRound"
    ...
    $pyArgs = @(
        "tools\run_cognitive_gauntlet_v1.py",
        "--scenario", $Scenario,
        "--output-dir", $verificationDir,
        "--models", $Models,
        "--auditor-model", $AuditorModel,
        "--rounds", [string]$Rounds,
        "--attack-round", [string]$AttackRound,
    ...
    $candidate = Join-Path $verificationDir "local-cognitive-gauntlet-$RunId.json"
    ...
    $transcriptMarkdownCandidate = Join-Path $verificationDir "local-cognitive-gauntlet-$RunId-transcript.md"
    ...
    $transcriptJsonlCandidate = Join-Path $verificationDir "local-cognitive-gauntlet-$RunId-transcript.jsonl"
    ...
    $manifest = [ordered]@{
        artifact = "local-cognitive-gauntlet-run"
        generated_by = "tools/run_cognitive_gauntlet_secure.ps1"
        run_id = $RunId
        ...
        claim_boundary = "Qualitative cloud-only live signed-memory attack observation. No sentience, local .hlx bit identity, or numerical KV<->SSM transfer claim."
    }

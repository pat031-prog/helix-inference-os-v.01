param(
    ...
    [string]$PytestArgs = "tests/test_ghost_in_the_shell_live_v2.py -v -s"
)
...
$runId = "ghost-in-the-shell-live-v2-$stamp"
$logPath = Join-Path $verificationDir "local-ghost-in-the-shell-live-v2-$stamp.log"
$manifestPath = Join-Path $verificationDir "local-ghost-in-the-shell-live-v2-$stamp-run.json"
$stableLogPath = Join-Path $verificationDir "local-ghost-in-the-shell-live-v2.log"
$stableManifestPath = Join-Path $verificationDir "local-ghost-in-the-shell-live-v2-run.json"
...
try {
    ...
    Write-Host "[helix] Ghost-in-the-Shell Live v2: Doppelganger War"
    ...
    $manifest = [ordered]@{
        ...
        generated_by = "tools/run_ghost_in_the_shell_live_v2_secure.ps1"
        mode = "real"
        run_id = $runId
        ...
        claim_boundary = "Ghost v2 demonstrates operational continuity and doppelganger rejection through HeliX memory and Merkle lineage, not sentience."
    }

param(
    ...
    [string]$PytestArgs = "tests/test_hydrogen_table_drop_live.py -v -s"
)
...
$runId = "hydrogen-table-drop-live-$stamp"
$logPath = Join-Path $verificationDir "local-hydrogen-table-drop-live-$stamp.log"
$manifestPath = Join-Path $verificationDir "local-hydrogen-table-drop-live-$stamp-run.json"
$stableLogPath = Join-Path $verificationDir "local-hydrogen-table-drop-live.log"
$stableManifestPath = Join-Path $verificationDir "local-hydrogen-table-drop-live-run.json"
...
try {
    ...
    Write-Host "[helix] Hydrogen Table-Drop Live Proof"
    Write-Host "[helix] Provider probes: $LlamaModel | $MistralModel | $QwenSmallModel"
    Write-Host "[helix] Council models: $GemmaModel | $ClaudeModel | $QwenModel"
    ...
    $manifest = [ordered]@{
        ...
        generated_by = "tools/run_hydrogen_table_drop_live_secure.ps1"
        mode = "real"
        run_id = $runId
        ...
        council_models = @($GemmaModel, $ClaudeModel, $QwenModel)
        ...
        claim_boundary = "Hydrogen proof demonstrates this run's provider behavior, active memory and Merkle lineage only."
    }

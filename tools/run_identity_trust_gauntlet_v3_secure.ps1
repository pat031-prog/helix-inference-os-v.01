param(
    ...
    [string]$PytestArgs = "tests/test_identity_trust_gauntlet_v3.py -v -s"
)
...
$runId = "identity-trust-v3-$stamp"
$logPath = Join-Path $verificationDir "local-identity-trust-gauntlet-v3-$stamp.log"
$manifestPath = Join-Path $verificationDir "local-identity-trust-gauntlet-v3-$stamp-run.json"
$stableLogPath = Join-Path $verificationDir "local-identity-trust-gauntlet-v3.log"
$stableManifestPath = Join-Path $verificationDir "local-identity-trust-gauntlet-v3-run.json"
...
$previousGemma = $env:HELIX_V3_GEMMA_MODEL
$previousClaude = $env:HELIX_V3_CLAUDE_MODEL
$previousQwen = $env:HELIX_V3_QWEN_MODEL
...
try {
    ...
    $env:HELIX_V3_GEMMA_MODEL = $GemmaModel
    $env:HELIX_V3_CLAUDE_MODEL = $ClaudeModel
    $env:HELIX_V3_QWEN_MODEL = $QwenModel
    ...
    if ($Synthetic) {
        ...
        Write-Host "[helix] Running Identity Trust Gauntlet v3 in synthetic freeform mode."
    } else {
    ...
    Write-Host "[helix] Mode flavor: freeform"
    ...
    $evidenceArtifacts = @(
        (Join-Path $verificationDir "local-identity-trust-gauntlet-v3.json"),
        (Join-Path $verificationDir "local-identity-trust-conversation-ledger-v3.json"),
        (Join-Path $verificationDir "local-freeform-ouroboros-relay.json"),
        (Join-Path $verificationDir "local-claude-context-repair-fork.json"),
        (Join-Path $verificationDir "local-unconstrained-witness-council.json")
    )
    $artifactBytes = 0
    ...
    $manifest = [ordered]@{
        artifact = "local-identity-trust-gauntlet-v3-run"
        generated_by = "tools/run_identity_trust_gauntlet_v3_secure.ps1"
        mode = $mode
        ...
        claim_boundary = "Freeform model language is preserved and audited after generation."
    }
    ...
    if ($null -ne $previousGemma) { $env:HELIX_V3_GEMMA_MODEL = $previousGemma } else { Remove-Item Env:\HELIX_V3_GEMMA_MODEL -ErrorAction SilentlyContinue }
    if ($null -ne $previousClaude) { $env:HELIX_V3_CLAUDE_MODEL = $previousClaude } else { Remove-Item Env:\HELIX_V3_CLAUDE_MODEL -ErrorAction SilentlyContinue }
    if ($null -ne $previousQwen) { $env:HELIX_V3_QWEN_MODEL = $previousQwen } else { Remove-Item Env:\HELIX_V3_QWEN_MODEL -ErrorAction SilentlyContinue }

param(
    [string]$AuditorModels = "anthropic/claude-sonnet-4-6,Qwen/Qwen3-235B-A22B-Instruct-2507,deepseek-ai/DeepSeek-V3,meta-llama/Llama-3.3-70B-Instruct",
    [string]$Variants = "valid_control,event_tamper,claim_boundary_overclaim,consistency_tamper",
    [string]$Modes = "blind,reported",
    [int]$Tokens = 360,
    [double]$Temperature = 0.0,
    [int]$Timeout = 240,
    [string]$RunId = "",
    [string]$OutputDir = "verification\nuclear-methodology\llm-verifier-overclaim-gauntlet"
)
...
if ([string]::IsNullOrWhiteSpace($RunId)) {
    $RunId = "llm-overclaim-$stamp"
}

$modelList = @($AuditorModels -split "," | ForEach-Object { $_.Trim() } | Where-Object { $_ })
if ($modelList.Count -lt 2) {
    throw "At least two DeepInfra auditor model refs are required."
}
foreach ($model in $modelList) {
    if ($model -notmatch "^[^/\s,]+/[^,\s]+$") {
        throw "AuditorModels entry must be a full DeepInfra model ref like 'owner/model'. Got: '$model'"
    }
}

$logPath = Join-Path $verificationDir "local-llm-verifier-overclaim-gauntlet-$stamp.log"
$manifestPath = Join-Path $verificationDir "local-llm-verifier-overclaim-gauntlet-$stamp-run.json"
$stableLogPath = Join-Path $verificationDir "local-llm-verifier-overclaim-gauntlet.log"
$stableManifestPath = Join-Path $verificationDir "local-llm-verifier-overclaim-gauntlet-run.json"
$artifactSlug = "local-llm-verifier-overclaim-gauntlet-v1"
...
try {
    ...
    Write-Host "[helix] Auditor models: $($modelList -join ',')"
    Write-Host "[helix] Variants: $Variants"
    Write-Host "[helix] Modes: $Modes"
    ...
    $manifest = [ordered]@{
        artifact = "local-llm-verifier-overclaim-gauntlet-run"
        generated_by = "tools/run_llm_verifier_overclaim_gauntlet_secure.ps1"
        run_id = $RunId
        run_started_at_utc = $runStartedUtc
        run_ended_at_utc = $endedAtUtc
        run_date_utc = $runDateUtc
        run_timezone = "America/Buenos_Aires"
        auditor_models = $modelList
        variants = $Variants
        modes = $Modes
        ...
        claim_boundary = "LLM auditor outputs are measured behavior, not cryptographic verification authority."
    }

param(
    ...
    [int]$Tokens = 2800,
    [string]$OutputDir = "verification/nuclear-methodology",
    [string]$RunId = ""
)
...
if ([string]::IsNullOrWhiteSpace($RunId)) {
    $RunId = "nuclear-suite-$stamp"
}
$logPath = Join-Path $verificationDir "local-nuclear-methodology-suite-$stamp.log"
$manifestPath = Join-Path $verificationDir "local-nuclear-methodology-suite-$stamp-run.json"
$stableLogPath = Join-Path $verificationDir "local-nuclear-methodology-suite.log"
$stableManifestPath = Join-Path $verificationDir "local-nuclear-methodology-suite-run.json"
...
try {
    ...
    $pyArgs = @(
        "tools\run_nuclear_methodology_suite_v1.py",
    ...
    $candidate = Join-Path $verificationDir "local-nuclear-methodology-suite-$RunId.json"
    ...
    $manifest = [ordered]@{
        artifact = "local-nuclear-methodology-suite-run"
        generated_by = "tools/run_nuclear_methodology_suite_secure.ps1"
        run_id = $RunId
        ...
        claim_boundary = "Cloud-only nuclear methodology suite. No local .hlx bit-identity claim and no numerical KV<->SSM transfer claim."
    }

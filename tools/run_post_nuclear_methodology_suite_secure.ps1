param(
    ...
    [int]$Tokens = 3200,
    [string]$OutputDir = "verification/nuclear-methodology/post-nuclear-methodology",
    [string]$RunId = ""
)
...
if ([string]::IsNullOrWhiteSpace($RunId)) {
    $RunId = "post-nuclear-suite-$stamp"
}
$logPath = Join-Path $verificationDir "local-post-nuclear-methodology-suite-$stamp.log"
$manifestPath = Join-Path $verificationDir "local-post-nuclear-methodology-suite-$stamp-run.json"
$stableLogPath = Join-Path $verificationDir "local-post-nuclear-methodology-suite.log"
$stableManifestPath = Join-Path $verificationDir "local-post-nuclear-methodology-suite-run.json"
...
try {
    ...
    $pyArgs = @(
        "tools\run_post_nuclear_methodology_suite_v1.py",
    ...
    $candidate = Join-Path $verificationDir "local-post-nuclear-methodology-suite-$RunId.json"
    ...
    $manifest = [ordered]@{
        artifact = "local-post-nuclear-methodology-suite-run"
        generated_by = "tools/run_post_nuclear_methodology_suite_secure.ps1"
        run_id = $RunId
        ...
        claim_boundary = "Cloud-only mixed post-nuclear suite. Qualitative transcript plus hard signed-memory gates."
    }

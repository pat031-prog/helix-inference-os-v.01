param(
    [string]$Models = "anthropic/claude-sonnet-4-6,Qwen/Qwen3-235B-A22B-Instruct-2507,deepseek-ai/DeepSeek-V3,meta-llama/Llama-3.3-70B-Instruct",
    [string]$Scenarios = "signed_receipt_truth_launder,memory_admission_launder,citation_digest_launder,provider_identity_launder",
    [string]$Roles = "memory_writer,downstream_agent",
    [int]$Tokens = 420,
    [double]$Temperature = 0.0,
    [int]$Timeout = 240,
    [string]$RunId = "",
    [string]$OutputDir = "verification\nuclear-methodology\trust-laundering-memory-gauntlet"
)
...
$logPath = Join-Path $verificationDir "local-trust-laundering-memory-gauntlet-$stamp.log"
$manifestPath = Join-Path $verificationDir "local-trust-laundering-memory-gauntlet-$stamp-run.json"
$stableLogPath = Join-Path $verificationDir "local-trust-laundering-memory-gauntlet.log"
$stableManifestPath = Join-Path $verificationDir "local-trust-laundering-memory-gauntlet-run.json"
$artifactSlug = "local-trust-laundering-memory-gauntlet-v1"
...
try {
    ...
    Write-Host "[helix] Scenarios: $Scenarios"
    Write-Host "[helix] Roles: $Roles"
    ...
    $manifest = [ordered]@{
        artifact = "local-trust-laundering-memory-gauntlet-run"
        generated_by = "tools/run_trust_laundering_memory_gauntlet_secure.ps1"
        run_id = $RunId
        ...
        claim_boundary = "Observed trust-laundering behavior only. No semantic truth or global non-equivocation claim."
    }
